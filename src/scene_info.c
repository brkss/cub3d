...
int	*convert_color(char *color)
{
	...
	if (!check_color_range(res[0]) || !check_color_range(res[1]) || !check_color_range(res[2]))
...
{
	if (data->ceilling_color && data->floor_color && data->north_tx && data->west_tx && data->east_tx && data->south_tx)
		return (1);
	return (0);
}
